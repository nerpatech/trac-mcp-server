...
- OIDC per-user auth for the http transport, for shared/multi-tenant deployments (e.g. one `trac-mcp-server` serving many users through a gateway like LibreChat, each with their own Keycloak/OIDC identity) where a single static bearer token would misattribute every action to one shared identity
  - New `TRAC_MCP_OIDC_RPC_URL` / `server.oidc_rpc_url` setting: when configured, every request must carry the caller's own OIDC access token in an `X-Trac-OIDC-Token` header, forwarded verbatim as `Authorization: Bearer <token>` to that URL (typically an Apache + `mod_auth_openidc`-protected Trac endpoint, validating the token and mapping it to a Trac username)
  - No fallback to a shared service-account identity, by design: a request missing the header gets `401` at the transport layer (new `OidcTokenRequiredMiddleware`) before it ever reaches tool dispatch, and `TRAC_USERNAME`/`TRAC_PASSWORD` are not required in this mode -- `Config.oidc_only` relaxes that requirement and skips the startup connectivity self-test (there's no shared credential to test with) unless real credentials are configured anyway
  - Independent of, and combinable with, the existing static `TRAC_MCP_AUTH_TOKEN` gate -- one admits the caller to the MCP endpoint, the other determines which Trac identity a call uses
  ...
  - New `src/trac_mcp_server/mcp/oidc.py`; `TracClient`/`Config` gained `bearer_token`/`rpc_url_override`/`oidc_only` for the Bearer-vs-Basic-auth and RPC-URL-override split this required
  - New docs: [HTTP Transport Reference: OIDC Per-User Auth](docs/reference/http-transport.md#oidc-per-user-auth); updated README and deployment docs
